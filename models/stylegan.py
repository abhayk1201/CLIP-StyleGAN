...
from models.stylegan_utils import FusedLeakyReLU, fused_leaky_relu, upfirdn2d
...
class Generator(nn.Module):
    ...
    def forward(
        ...
        # StyleGAN noise (across resolution layers)
        if noise is None:
            if randomize_noise:
                noise = [None] * self.num_layers
            elif len(noise_tensor_ll):
                assert len(noise_tensor_ll) == self.num_layers
                noise = noise_tensor_ll
            else:
                noise = [
